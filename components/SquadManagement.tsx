...
const SquadManagement: React.FC<SquadManagementProps> = ({ state, setState, onOpenReport, addLog }) => {
  ...
  const inputClass = "w-full bg-[#0f172a] border border-white/10 rounded-xl py-3 px-4 text-white focus:border-orange-500 outline-none transition-all placeholder:text-slate-600 text-sm font-bold";
  const labelClass = "text-[10px] font-black text-slate-400 uppercase mb-1.5 block flex items-center gap-2 tracking-widest";
  const sectionHeader = "text-orange-500 font-black text-[11px] uppercase tracking-[0.2em] border-r-4 border-orange-500 pr-3 mb-6 mt-8 flex items-center gap-2 bg-orange-500/5 py-2 rounded-l-lg";

  return (
    <div className="space-y-8 relative font-['IBM_Plex_Sans_Arabic']" dir="rtl">
      {/* شريط البحث والفلترة */}
      <div className="modern-card p-6 flex flex-col md:flex-row gap-4 items-center">
         <div className="flex-1 relative w-full">
            ...
            <input type="text" placeholder="البحث بالاسم أو الرقم..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              className="w-full bg-[#0f172a] border border-white/5 rounded-2xl py-4 pr-12 pl-6 text-white outline-none focus:border-orange-500 transition-all font-bold" />
         </div>

         {!restrictedCat ? (
           <div className="relative w-full md:w-64">
             <Filter className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
             <select 
               value={localCategoryFilter} 
               onChange={e => setLocalCategoryFilter(e.target.value)}
               className="w-full bg-[#0f172a] border border-white/5 rounded-2xl py-4 pr-12 pl-6 text-white outline-none focus:border-orange-500 transition-all font-black text-xs cursor-pointer appearance-none"
             >
           ...
           <div className="bg-orange-500/10 border border-orange-500/20 px-6 py-4 rounded-2xl text-orange-500 font-black text-xs uppercase tracking-widest">
              فئة: {restrictedCat}
           </div>
         )}
         ...
         {!isViewer && (
           <button onClick={() => { setEditingId(null); setFormData(initialFormState); setIsModalOpen(true); }}
             className="bg-orange-500 text-white px-8 py-4 rounded-2xl font-black flex items-center gap-3 shadow-lg hover:bg-orange-600 transition-all whitespace-nowrap w-full md:w-auto justify-center">
      ...
      <div className="flex p-1.5 bg-[#0f172a] border border-white/5 rounded-2xl w-fit">
          <button onClick={() => setActiveSubTab('players')} className={`px-10 py-3 rounded-xl font-black text-xs transition-all ${activeSubTab === 'players' ? 'bg-orange-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>اللاعبين ({state.people.filter(p => p.role==='لاعب' && (!restrictedCat || p.category===restrictedCat)).length})</button>
          <button onClick={() => setActiveSubTab('staff')} className={`px-10 py-3 rounded-xl font-black text-xs transition-all ${activeSubTab === 'staff' ? 'bg-orange-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>الكوادر ({state.people.filter(p => p.role!=='لاعب' && (!restrictedCat || p.category===restrictedCat)).length})</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredMembers.map(person => (
          <div key={person.id} className="modern-card p-8 group hover:border-orange-500/50 transition-all flex flex-col border-b-4 border-b-orange-500/20">
             <div className="flex justify-between items-start mb-6">
                <div className="w-16 h-16 bg-slate-800 rounded-2xl flex items-center justify-center font-black text-3xl text-white border-2 border-orange-500/20 shadow-xl group-hover:bg-orange-500 transition-colors">
             ...
             <h3 className="text-xl font-black text-white mb-2">{person.name} {person.number && <span className="text-orange-500 text-sm">#{person.number}</span>}</h3>
             <p className="text-[10px] text-slate-500 font-bold mb-6">{person.position || 'بدون مركز محدد'}</p>
        ...
        {filteredMembers.length === 0 && (
          <div className="col-span-full py-20 text-center border-4 border-dashed border-white/5 rounded-[3rem] opacity-30">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#0f172a] border-4 border-slate-700 w-full max-w-5xl max-h-[90vh] rounded-[2.5rem] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-white/10 bg-[#1e293b] flex justify-between items-center">
              <h3 className="text-xl font-black text-white flex items-center gap-3"><UserPlus className="text-orange-500" /> {editingId ? 'تعديل السجل المركزي' : 'إضافة سجل جديد لنادي الكرامة'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="p-2 bg-red-50/10 hover:bg-red-500 text-red-500 hover:text-white rounded-xl transition-all"><X size={24}/></button>
            </div>

            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-10 space-y-2 custom-scrollbar bg-[#020617] text-right" dir="rtl">
              
              {/* القسم 1: الهوية */}
              <h4 className={sectionHeader}><Fingerprint size={16}/> 1. معلومات الهوية الشخصية والقيود</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1"><label className={labelClass}>الاسم الثلاثي الكامل</label><input required className={inputClass} value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} /></div>
                ...
                <div className="space-y-1"><label className={labelClass}>الخانة (القيد المدني)</label><input className={inputClass} value={formData.khana || ''} onChange={e => setFormData({...formData, khana: e.target.value})} /></div>
                ...
                <div className="space-y-1"><label className={labelClass}>الرقم الاتحادي (سوري)</label><input className={inputClass} value={formData.federalNumber || ''} onChange={e => setFormData({...formData, federalNumber: e.target.value})} /></div>
                <div className="space-y-1"><label className={labelClass}>الرقم الدولي (FIFA/ID)</label><input className={inputClass} value={formData.internationalId || ''} onChange={e => setFormData({...formData, internationalId: e.target.value})} /></div>
              </div>

              {/* القسم 2: الرياضة */}
              <h4 className={sectionHeader}><Activity size={16}/> 2. البيانات الرياضية والفنية</h4>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="space-y-1"><label className={labelClass}>تاريخ الانضمام للنادي</label><input type="date" className={inputClass} value={formData.joinDate || ''} onChange={e => setFormData({...formData, joinDate: e.target.value})} /></div>
                ...
                <div className="space-y-1 md:col-span-2"><label className={labelClass}>التحصيل العلمي (الدرجة العلمية)</label><input className={inputClass} value={formData.academicDegree || ''} onChange={e => setFormData({...formData, academicDegree: e.target.value})} /></div>
                ...
                <div className="space-y-1 md:col-span-4"><label className={labelClass}>عنوان السكن التفصيلي</label><input className={inputClass} value={formData.address || ''} onChange={e => setFormData({...formData, address: e.target.value})} /></div>
              </div>

              {/* القسم 3: العقود */}
              <h4 className={sectionHeader}><Wallet size={16}/> 3. السجل التعاقدي والقيمة المالية</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-1"><label className={labelClass}>قيمة العقد / الراتب الشهري</label><input className={inputClass} value={formData.contractValue || ''} onChange={e => setFormData({...formData, contractValue: e.target.value})} /></div>
              </div>

              {/* القسم 4: الطبي والانضباط */}
              <h4 className={sectionHeader}><ShieldAlert size={16}/> 4. الحالة الطبية والسلوك الانضباطي</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1"><label className={labelClass}>تاريخ العمليات/السجل الطبي</label><textarea className={`${inputClass} h-24`} value={formData.medicalHistory || ''} onChange={e => setFormData({...formData, medicalHistory: e.target.value})} /></div>
                <div className="space-y-1"><label className={labelClass}>الإصابات المزمنة الحالية</label><textarea className={`${inputClass} h-24`} value={formData.injuries || ''} onChange={e => setFormData({...formData, injuries: e.target.value})} /></div>
                <div className="space-y-1"><label className={labelClass}>العقوبات والسوابق الانضباطية</label><textarea className={`${inputClass} h-24`} value={formData.penalties || ''} onChange={e => setFormData({...formData, penalties: e.target.value})} /></div>
                <div className="space-y-1"><label className={labelClass}>ملاحظات إضافية للمدرب</label><textarea className={`${inputClass} h-24`} value={formData.notes || ''} onChange={e => setFormData({...formData, notes: e.target.value})} /></div>
              </div>
