...
const SettingsView: React.FC<SettingsProps> = ({ state, setState }) => {
  ...
  const handleManualSync = async () => {
    setIsSyncing(true);
    // المحاكاة لعملية المزامنة اليدوية (المزامنة الحقيقية تتم تلقائياً في App.tsx)
    await new Promise(r => setTimeout(r, 2000));
    setIsSyncing(false);
    alert('تم تحديث المزامنة مع قاعدة بيانات Supabase بنجاح.');
  };
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 relative z-10">
          <div className="flex items-center gap-8">
            ...
            <div className="space-y-2">
              <h3 className="text-3xl font-black text-slate-800">قاعدة البيانات السحابية</h3>
              <p className={`text-sm font-bold leading-relaxed text-slate-500`}>
                نظام Supabase يقوم بحفظ بيانات الكادر واللاعبين والمباريات بشكل فوري وآمن.
              </p>
            </div>
          </div>
          <button 
          ...
          >
            {isSyncing ? <RefreshCw className="animate-spin" /> : <Cloud />} مزامنة سحابية يدوية
          </button>
        </div>

        <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-4">
           <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-200 text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase">سجلات الأعضاء</p>
              <p className="text-2xl font-black text-blue-900">{state.people.length}</p>
           </div>
           <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-200 text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase">سجلات الحضور</p>
              <p className="text-2xl font-black text-emerald-600">{state.attendance.length}</p>
           </div>
           <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-200 text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase">آخر مزامنة</p>
              <p className="text-xs font-black text-slate-800">{state.lastSyncTimestamp ? new Date(state.lastSyncTimestamp).toLocaleTimeString('ar-EG') : 'جاري المزامنة...'}</p>
           </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-[3.5rem] w-full max-w-lg shadow-2xl overflow-hidden p-8 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input required type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full bg-slate-100 rounded-2xl p-4 font-black text-slate-950 outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div className="space-y-1">
